...
const SORTABLE_FIELDS = ["createdAt", "price", "title"] as const;
...
type ListingBodyInput = Record<string, any>;
...
const serializeListingMedia = (baseUrl: string, listing: any): any => {
  if (!listing) {
    return listing;
  }
...
const isRenewableListing = (listing: any): boolean => Boolean(
...
const getServicePurchaseContext = (listingId: string, userId: string, planId: string): Promise<[any, any, any, any]> =>
  Promise.all([
...
const expireServiceListings = async (): Promise<void> => {
  const now = new Date();
...
class ListingService {
  async createService({ body, files, userId, baseUrl }: { body: ListingBodyInput; files: UploadedFiles; userId: string; baseUrl: string }): Promise<ServiceResult> {
    try {
      const {
        title,
        description,
  ...
  async getPublicServices({ query, baseUrl }: { query: QueryInput; baseUrl: string }): Promise<ServiceResult> {
    try {
      ...
      return {
        ...
        data: services.map((service: any) => serializeListingMedia(baseUrl, service)),
        meta: {
          total,
          page: parsedPage,
          limit: parsedLimit,
  ...
  async getServiceById({ id, baseUrl }: { id: string; baseUrl: string }): Promise<ServiceResult> {
    try {
      await expireServiceListings();
  ...
  async getMyServices({ userId, query = {}, baseUrl }: { userId: string; query?: QueryInput; baseUrl: string }): Promise<ServiceResult> {
    try {
      await expireServiceListings();
  ...
  async deleteMyService({ id, userId }: { id: string; userId: string }): Promise<ServiceResult> {
    try {
      await expireServiceListings();
  ...
  async getAdminServices({ query, baseUrl }: { query: QueryInput; baseUrl: string }): Promise<ServiceResult> {
    try {
      await expireServiceListings();
  ...
  async updateServiceStatus({ id, status, baseUrl }: { id: string; status: string; baseUrl: string }): Promise<ServiceResult> {
    try {
      await expireServiceListings();
  ...
  async reportServiceSpam({ id, baseUrl }: { id: string; baseUrl: string }): Promise<ServiceResult> {
    try {
      const existingListing = await prisma.listing.findFirst({
        where: {
          id,
  ...
  async deleteService({ id }: { id: string }): Promise<ServiceResult> {
    try {
      await expireServiceListings();
  ...
  async createServiceRenewalCheckoutSession({
    listingId,
    userId,
    planId,
    successUrl,
  ...
  async confirmServiceListingPurchase({
    listingId,
    userId,
    planId,
    checkoutSessionId,
