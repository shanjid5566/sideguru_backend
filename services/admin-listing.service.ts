...
const normalizeError = (error: unknown): HttpError => {
  const typed = error as { meta?: { cause?: string }; statusCode?: number; message?: string };

  if (typed?.meta?.cause) {
    return createHttpError(500, typed.meta.cause);
  }

  if (typed?.statusCode && typed?.message) {
    return typed as HttpError;
  }

  return createHttpError(500, typed?.message || "An unexpected error occurred");
};

const expireListings = async (): Promise<void> => {
  try {
    const now = new Date();
...
const getListingExpiryDate = (): Date => {
...
class AdminListingService {
  async getAllListings({ query, baseUrl }: { query: QueryInput; baseUrl: string }) {
    try {
      ...
      if (listingType && !["SERVICE", "EVENT"].includes(listingType)) {
        throw createHttpError(400, "Invalid listingType value. Must be SERVICE or EVENT");
      }
      ...
      return {
        ...
        data: listings.map((listing: any) => serializeListingMedia(baseUrl, listing)),
        meta: {
          total,
          page: parsedPage,
          limit: parsedLimit,
  ...
  async getListingById({ id, baseUrl }: { id: string; baseUrl: string }) {
    try {
      await expireListings();
  ...
  async updateListingStatus({ id, status, baseUrl }: { id: string; status: string; baseUrl: string }) {
    try {
      await expireListings();
