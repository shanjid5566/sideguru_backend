...
const SUPPORTED_PERIODS = ["thisYear", "lastYear", "thisMonth", "lastMonth", "last3Months", "last6Months"];
const getMonthRange = (date = new Date(), monthOffset = 0) => {
    const reference = new Date(date.getFullYear(), date.getMonth() + monthOffset, 1);
    const start = new Date(reference.getFullYear(), reference.getMonth(), 1);
    const end = new Date(reference.getFullYear(), reference.getMonth() + 1, 1);
    return { start, end };
};
const getYearRange = (date = new Date(), yearOffset = 0) => {
    const year = date.getFullYear() + yearOffset;
    const start = new Date(year, 0, 1);
    const end = new Date(year + 1, 0, 1);
    return { start, end, year };
};
const shiftRange = ({ start, end }, amount, unit) => {
...
const getRangeForPeriod = (period, now = new Date()) => {
    switch (period) {
        ...
        default:
            throw (0, httpError_1.createHttpError)(400, `period must be one of: ${SUPPORTED_PERIODS.join(", ")}`);
    }
};
const buildChartBuckets = ({ currentRange, bucketType, bucketCount }) => {
    if (bucketType === "day") {
...
const applyPaymentsToBuckets = (payments, buckets) => {
...
const toPercentageChange = (currentValue, previousValue) => {
    if (previousValue === 0) {
...
class DashboardService {
    async getAdminOverview({ period = "thisYear" } = {}) {
        const now = new Date();
        const selectedPeriod = (period || "thisYear");
        ...
        const [currentUsers, previousUsers, currentListings, previousListings, currentRevenueAggregate, previousRevenueAggregate, currentPendingListings, previousPendingListings, currentSpamAggregate, previousSpamAggregate, successfulPayments, currentServiceListings, currentEventListings,] = await Promise.all([
