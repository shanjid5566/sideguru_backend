...
const isRenewableListing = (listing) => {
    ...
    const now = new Date();
    return Boolean(listing.status === "EXPIRED"
...
const parseDateInput = (value, fieldName) => {
    ...
    if (Number.isNaN(parsedDate.getTime())) {
        throw (0, httpError_1.createHttpError)(400, `${fieldName} must be a valid date`);
    }
    return parsedDate;
};
...
class EventService {
    async createEvent({ body, files, userId, baseUrl }) {
        try {
            const { title, description, price, categoryId, countryId, regionId, address, location, contactEmail, contactPhone, email, phone, facebookUrl, instagramUrl, facebook, instagram, mainImage, eventImage, eventImages, gallery, eventGallery, startDate, endDate, eventStart, eventEnd, } = body;
            ...
            if (!title || !description || !categoryId || !countryId || !regionId || !resolvedContactEmail || !resolvedContactPhone || !resolvedMainImage || !resolvedStartDateInput || !resolvedEndDateInput) {
                throw (0, httpError_1.createHttpError)(400, "title, description, categoryId, countryId, regionId, contactEmail, contactPhone, startDate, endDate, and event image are required");
            }
            ...
            if (parsedEndDate <= parsedStartDate) {
                throw (0, httpError_1.createHttpError)(400, "endDate must be later than startDate");
            }
            ...
            if (!category) {
                throw (0, httpError_1.createHttpError)(400, "Invalid event category");
            }
            if (!country) {
    ...
    async getEventById({ id, baseUrl }) {
        try {
            ...
            if (!listing || listing.deletedAt || listing.listingType !== "EVENT" || listing.status !== "APPROVED" || !listing.expiresAt || listing.expiresAt <= new Date()) {
                throw (0, httpError_1.createHttpError)(404, "Event not found");
            }
            return {
                statusCode: 200,
    ...
    async updateEvent({ id, userId, body, files, baseUrl, }) {
        try {
            ...
            if (!existingEvent) {
                throw (0, httpError_1.createHttpError)(404, "Event not found or you do not have permission to edit it");
            }
            const updateData = {};
            if (body.title)
    ...
    async deleteMyEvent({ id, userId }) {
        try {
            ...
            if (!existingEvent) {
                throw (0, httpError_1.createHttpError)(404, "Event not found or you do not have permission to delete it");
            }
    ...
    async createEventPaymentIntent({ listingId, userId, planId, successUrl, cancelUrl, baseUrl, }) {
        try {
            ...
            if (!listing) {
                throw (0, httpError_1.createHttpError)(404, "Event listing not found");
            }
            if (!plan) {
            ...
            if (!isRenewal && listing.payments?.some((payment) => payment.status === "SUCCESS")) {
                throw (0, httpError_1.createHttpError)(409, "This event listing has already been paid for");
            }
    ...
    async createEventRenewalCheckoutSession({ listingId, userId, planId, successUrl, cancelUrl, baseUrl, }) {
        try {
            ...
            if (!isRenewableListing(listing)) {
                throw (0, httpError_1.createHttpError)(400, "Only expired events can use the renew checkout endpoint");
            }
    ...
    async confirmEventListingPurchase({ listingId, userId, planId, checkoutSessionId, baseUrl, }) {
        try {
            ...
            if (metadata.listingId !== listing.id || metadata.planId !== plan.id || metadata.userId !== userId) {
                throw (0, httpError_1.createHttpError)(400, "Checkout session does not match this event listing purchase request");
            }
    ...
    async confirmEventRenewal({ listingId, userId, planId, checkoutSessionId, baseUrl, }) {
        try {
            ...
            if (!isRenewableListing(listing)) {
                throw (0, httpError_1.createHttpError)(400, "Only expired events can use the renew confirmation endpoint");
            }
